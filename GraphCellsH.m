// GraphCellsH.m
//
// Compute the p-canonical basis for a finite Coxeter group and print the
// two-sided p-cells as R-cell by L-cell intersection matrices.
//
// Typical usage:
//   magma -b type:=G2 char:=2 saveDir:=saves GraphCellsH.m > g-2-2.txt
//   magma -b type:=F4 char:=3 saveDir:=saves GraphCellsH.m > f-4-3.txt
//
// The output lists each two-sided p-cell J, the matrix
//      ( |R_i \cap L_j| )_{i,j},
// and, for each diagonal H-cell R_i \cap L_i, the multiplication table in the
// p-canonical basis restricted to that H-cell.
//
// Requirements:
//   - ASLoc.spec must be visible from the working directory.
//   - The IHecke package must be available through ASLoc.
//   - If saveDir is used, create the directory first.
//
// Notes:
//   - char must be 0 or a prime.
//   - targetLength can be used for partial tests, but for cell computations
//     one usually wants the full finite Coxeter group.
//   - saveDir stores/resumes serialized p-canonical basis data.
...
procedure Usage(missingArg)
    printf "Error: %o\n", missingArg;
    print "";
    print "Usage example: magma -b type:=G2 char:=2 saveDir:=saves GraphCellsH.m > g-2-2.txt";
    print "";
    print "Arguments:";
...
if assigned profileName then
    ...
    procedure WriteProfile()
        name := Sprintf("GraphCellsH-Profile-%o-%o-%o-", type, char, profileName);
...
// Set chattiness.
chatty := assigned chatty select StringToInteger(chatty) else 0;
...
// Set up the diagrammatic category and the Hecke algebra.
W := CoxeterGroup(GrpFPCox, cartanMat);
B := BSParabolic(cartanMat, W, []);
HAlg, H, C := ShortcutIHeckeAlgebra(W);

// By default we calculate all elements of the finite Weyl group.
...
print "Complete p-canonical basis for", W;
...
// Compute all three kinds of cells
left, right, twoSided := Cells(pC);

// ===== Cell-shape matrices (rows = R-cells, columns = L-cells) =====

// Short word formatter: [] ↦ "id", else concatenated digits.
function FmtElt(w)
    return #w eq 0 select "id" else &cat[ IntegerToString(i) : i in Eltseq(w) ];
end function;

// Pick a readable representative from a set of words: first by length, then lex.
function RepWord(S)
    seq := Setseq(S);
    Sort(~seq,
        func<x,y |
            (#Eltseq(x) ne #Eltseq(y)) select (#Eltseq(x) - #Eltseq(y))
            else (Eltseq(x) lt Eltseq(y) select -1 else (Eltseq(x) eq Eltseq(y) select 0 else 1))
        >);
    return seq[1];
end function;

// Build and print the R×L intersection matrix for one J-cell
procedure PrintCellPictureForJ(Jset, left, right, jIndex)
    J := Set(Jset);

    // Restrict L- and R-cells to this J: keep only nonempty intersections
    Lfull := Components(left);
    Rfull := Components(right);
    Ls := [ Set(Lc) meet J : Lc in Lfull | #(Set(Lc) meet J) gt 0 ];
    Rs := [ Set(Rc) meet J : Rc in Rfull | #(Set(Rc) meet J) gt 0 ];

    // Optional ordering: by size (ascending). Change comparator to flip.
    Sort(~Ls, func<a,b | #a - #b>);
    Sort(~Rs, func<a,b | #a - #b>);

    // Build the matrix of sizes |R_i ∩ L_j|
    M := [ [ #(Rs[i] meet Ls[j]) : j in [1..#Ls] ] : i in [1..#Rs] ];

    // Column/row labels via compact representatives
    Llab := [ FmtElt(RepWord(Ls[j])) : j in [1..#Ls] ];
    Rlab := [ FmtElt(RepWord(Rs[i])) : i in [1..#Rs] ];

    // Pretty-print
    printf "==========\n";
    printf "JCell #%o: size %o  (R-cells: %o, L-cells: %o)\n", jIndex, #J, #Rs, #Ls;

    // Matrix (space-separated). Use fixed width if you prefer alignment.
    for i in [1..#Rs] do
        printf "%o\n", Join([ IntegerToString(x) : x in M[i] ], " ");
    end for;

    // Legend (compact): which col/row corresponds to which L/R-cell (by a rep element)
    printf "Columns = L-cells (reps): %o\n", Join(Llab, " | ");
    printf "Rows    = R-cells (reps): %o\n\n", Join(Rlab, " | ");

    // ===== Diagonal H-cells and full pC-multiplication table (no v=1) =====
    printf "Diagonal H-cells (R_i ∩ L_i):\n";

    for k in [1..Min(#Rs, #Ls)] do
        // Elements on the diagonal H-cell: R_k ∩ L_k
        Hlist := Setseq(Rs[k] meet Ls[k]);

        // Sort like your reps: by word length, then lex on Eltseq
        Sort(~Hlist,
            func<x,y |
                (#Eltseq(x) ne #Eltseq(y)) select (#Eltseq(x) - #Eltseq(y))
                else (Eltseq(x) lt Eltseq(y) select -1 else (Eltseq(x) eq Eltseq(y) select 0 else 1))
        ...
        // Quick label list and membership set
        labels := [ FmtElt(w) : w in Hlist ];
        HsetS  := SequenceToSet(Hlist);

        printf "  Diagonal (%o , %o):\n", Rlab[k], Llab[k];
        printf "    Elements: {%o}\n", Join(labels, ", ");
        printf "    Multiplication table in pC (rows u, cols v; restricted to this H-cell):\n";

        // Print all products pC(u)*pC(v), expressed in pC with full polynomial coefficients,
        // keeping only summands whose basis element lies in this H-cell.
        for u in Hlist do
            row := [ ];
            for v in Hlist do
                prod_pC := pC ! ( (pC.u) * (pC.v) );
                supp := [ y : y in Support(prod_pC) | y in HsetS ];
                terms := [ ];
                for y in supp do
                    c := Coefficient(prod_pC, y);     // Laurent polynomial in v
                    if c ne 0 then
                        Append(~terms, Sprintf("(%o)·pC(%o)", c, FmtElt(y)));
                    end if;
                end for;
                entry := (#terms eq 0) select "0" else Join(terms, " + ");
                Append(~row, entry);
            end for;
            printf "      %o : %o\n", FmtElt(u), Join(row, " | ");
        end for;
...
// Print a picture for every J-cell, ordered as in Components(twoSided)
Jcomps := Components(twoSided);
for j -> Jset in Jcomps do
    PrintCellPictureForJ(Jset, left, right, j);
end for;
